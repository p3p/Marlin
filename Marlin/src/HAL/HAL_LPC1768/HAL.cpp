...
struct MedianFilter {
  uint16_t values[ADC_MEDIAN_FILTER_SIZE];
  uint8_t next_val;
  ...
  uint16_t update(uint16_t value) {
    uint16_t sorted_values[ADC_MEDIAN_FILTER_SIZE];
    
    values[next_val++] = value;
    next_val = next_val % num_values;
    
    memcpy(sorted_values, values, sizeof(sorted_values));
    std::sort(sorted_values, sorted_values + ADC_MEDIAN_FILTER_SIZE);
    
    return sorted_Values[(ADC_MEDIAN_FILTER_SIZE / 2) + 1];
  }
};
