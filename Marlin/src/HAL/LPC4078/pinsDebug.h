...
#define printPinNumber(p) do{ sprintf_P(buffer, PSTR("P%d_%02d"), MCUI::pin_port(p), MCUI::pin_bit(p)); SERIAL_ECHO(buffer); }while(0)
#define printPinAnalog(p) do{ sprintf_P(buffer, PSTR("_A%d     "), MCUI::pin_get_adc_channel(pin)); SERIAL_ECHO(buffer); }while(0)
