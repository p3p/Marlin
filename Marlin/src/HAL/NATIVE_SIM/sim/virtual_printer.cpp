...
void VirtualPrinter::build() {
  ...
  root->add_component<Heater>("Bed Heater", HEATER_BED_PIN, TEMP_BED_PIN, heater_data{12, 1.2}, hotend_data{325, 824, 0.897}, adc_data{4700, 12});
  #if HAS_SPI_FLASH
    root->add_component<W25QxxDevice>("SPI Flash", SCK_PIN, MISO_PIN, MOSI_PIN, W25QXX_CS_PIN, SPI_FLASH_SIZE);
  #endif
  #ifdef SDSUPPORT
    root->add_component<SDCard>("SD Card", SCK_PIN, MISO_PIN, MOSI_PIN, SDSS, SD_DETECT_PIN, SD_DETECT_STATE);
  #endif
    ...
    root->add_component<ST7796Device>("ST7796Device Display", SCK_PIN, MISO_PIN, MOSI_PIN, TFT_CS_PIN, TOUCH_CS_PIN, TFT_DC_PIN, BEEPER_PIN, BTN_EN1, BTN_EN2, BTN_ENC, BTN_BACK, KILL_PIN);
